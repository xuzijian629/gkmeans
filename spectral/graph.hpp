/*
    This file is almost same with /kmeans/graph.hpp.
    It inverts the weight of each edge so that it express the similarity, not the distance.
    Also, we set the default value of the adjacency matrix
*/
...
struct Graph {
    ...
    Graph(int n, bool use_adj_mat = false) : n(n), use_adj_mat(use_adj_mat) {
        if (use_adj_mat) {
            adj_mat = vector<vector<double>>(n, vector<double>(n));
            for (int i = 0; i < n; i++) {
                adj_mat[i][i] = 0;
            }
        } else {
